...
// GAP GATT Attributes
static uint8 attDeviceName[GAP_DEVICE_NAME_LEN] = "PDF_SER";
...
static void simpleBLEPeripheral_HandleKeys( uint8 shift, uint8 keys )
{
	...
	{
		i = 1;
		SimpleProfile_SetParameter( SIMPLEPROFILE_CHAR4, sizeof(uint8), &i);
...
static void NpiSerialCallback( uint8 port, uint8 events )
{
	...
	if (events & (HAL_UART_RX_TIMEOUT | HAL_UART_RX_FULL))   //´®¿ÚÓÐÊý¾Ý
	{
		...
		if(numBytes)
		{
			NPI_ReadTransport(buf,numBytes);    //´Ó´®¿Ú¶Á³öÊý¾Ý
		}
	}
}
